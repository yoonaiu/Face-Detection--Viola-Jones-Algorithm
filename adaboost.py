...
class Adaboost:
    ...
    def train(self, dataset):
        ...
        #print( utils.integralImage(dataset[i][0]).shape )
        ...
        for t in range(self.T):
            ...
            # Normalize weights
            # weights = weights / np.linalg.norm(weights) -> the line below
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        """
        function detail:
          1. for each Haar like feature, train as a WeakClassifier.
             
          2. You can obtain the value that each feature count with each img simply from featureVals
             , or from classifier.classify(iis[img_id]).
            
          3. Derive error for each Haar like feature from the counting through all imgs with weight(of each feature), 
             and at the end return the minimum error one,
             with its error and its "object" of WeakClassifier class.

        Returns:
          bestClf: need to be a "class"
        """
        ...
        for feature_id in range(len(features)):
          error = 0
          classifier = WeakClassifier(features[feature_id])
          for img_id in range(len(labels)):
            val = 0
            if featureVals[feature_id][img_id] < 0:
              val = 1
            
            if abs(val - labels[img_id]) == 1:
              error += weights[img_id]

          if error < bestError or bestError == -1:
            bestError = error
            bestClf = classifier
        ...
        for feature_id in range(len(features)):
          ...
          for img_id in range(len(labels)):
            val = 0
            x = featureVals[feature_id][img_id]
            if 2*x + 3 < 0:
              val = 1
        ...
        """
        candidate:
        # error += ( weights[img_id] )*( abs(val - labels[img_id]) )
        # error += ( weights[img_id] ** 2 )*( abs(val - labels[img_id]) )
        """
